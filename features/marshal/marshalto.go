...
func (p *marshal) encodeVarint(varName ...string) {
	p.P(`i = `, commonPackage.Ident("EncodeVarint"), `(dAtA, i, uint64(`, strings.Join(varName, ""), `))`)
}
...
func (p *marshal) field(oneof bool, numGen *counter, field *protogen.Field) {
	...
	case protoreflect.Int64Kind, protoreflect.Uint64Kind, protoreflect.Int32Kind, protoreflect.Uint32Kind, protoreflect.EnumKind:
		if packed {
			...
			p.P(total, ` += `, commonPackage.Ident("Sov"), `(uint64(num))`)
			p.P(`}`)
	...
	case protoreflect.Sint32Kind:
		if packed {
			...
			p.P(`var `, total, ` int`)
			p.P(`for _, num := range m.`, fieldname, ` {`)
			p.P(total, ` += `, commonPackage.Ident("Soz"), `(uint64(num))`)
			p.P(`}`)
			p.P(`i -= `, total)
			p.P(jvar, `:= i`)
...
func (p *marshal) message(message *protogen.Message) {
	...
	if p.strict {
		...
		// before fields. See https://git.woa.com/trpc-vtproto/common/vtprotobuf/pull/22
